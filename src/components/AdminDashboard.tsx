  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0B192C] via-[#102945] to-[#063B3B] text-white p-5 sm:p-7 rounded-[2rem] border border-slate-700/70 shadow-2xl">
        <div className="absolute -top-20 -left-16 w-56 h-56 rounded-full bg-brand-400/10 blur-3xl" />
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-brand-200 font-black"><Activity className="w-3.5 h-3.5" /> النظام يعمل</span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="text-[10px] text-brand-600 font-semibold mt-1">✓ كل الأطوار الدراسية</div>
        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {l.status === 'active' ? (
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-brand-50 hover:bg-brand-100 text-brand-700 font-bold"
                              title="تفعيل الإعلان"
                            >
                      ...
                      {r.status === 'pending' ? (
                        <>
                          <button
                            ...
                            className="bg-brand-600 text-white font-bold px-3 py-1.5 rounded-xl shadow text-xs"
                          >
                            معالجة وإغلاق
                          </button>
                          <button
                        ...
                        <span className="text-brand-700 font-bold bg-brand-100 px-2 py-1 rounded-lg">
                          تم الحل ✓
                        </span>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <input value={userQuery} onChange={(event) => setUserQuery(event.target.value)} placeholder="ابحث بالاسم أو البريد" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pr-9 pl-3 text-xs outline-none focus:border-brand-500" />
              </div>
            </div>
              ...
              <div className="divide-y divide-slate-100">
                {filteredUsers.map((user) => (
                  <div key={user.id} className="p-4 flex items-center justify-between gap-3 hover:bg-slate-50 transition-colors">
                    ...
                    <span className={`text-[10px] font-black px-2.5 py-1 rounded-full shrink-0 ${user.role === 'admin' ? 'bg-amber-100 text-amber-800' : 'bg-brand-100 text-brand-800'}`}>{user.role === 'admin' ? 'مسؤول' : 'مستخدم'}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {EDUCATION_LEVELS.map(lvl => (
                <div key={lvl.id} className="p-4 rounded-2xl border border-slate-200 bg-slate-50 space-y-2 text-xs">
                  <div className="font-bold text-sm text-brand-950 flex items-center justify-between">
              ...
              {WILAYAS.map(w => {
                ...
                return (
                  <div key={w.code} className="p-2.5 rounded-xl border border-slate-200 bg-slate-50 flex items-center justify-between">
                    ...
                    <span className={`font-black text-xs px-2 py-0.5 rounded-md ${count > 0 ? 'bg-brand-100 text-brand-800' : 'text-slate-300'}`}>
                      {count}
                    </span>
                  </div>
                );
        ...
        {activeTab === 'monetization' && (
          <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              ...
              <div className="p-5 rounded-3xl border-2 border-brand-300 bg-brand-50/40 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-brand-900 text-sm">اشتراك المكتبات المعتمدة</span>
                  <Building2 className="w-5 h-5 text-brand-600" />
                </div>
                <div className="text-xl font-black text-brand-950 font-mono">

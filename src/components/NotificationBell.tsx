...
export const NotificationBell: React.FC<NotificationBellProps> = ({ userId }) => {
  ...
  return <div ref={ref} className="relative">
    ...
    {open && <div className="absolute left-0 top-12 z-50 w-[min(360px,calc(100vw-2rem))] rounded-2xl bg-white text-slate-900 border border-slate-200 shadow-2xl overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex items-center justify-between"><div><h3 className="font-black text-sm">الإشعارات</h3><p className="text-[11px] text-slate-400 mt-1">{unread ? `${unread} غير مقروءة` : 'كل شيء محدث'}</p></div><button onClick={markAll} disabled={!unread || loading} className="text-[11px] text-emerald-700 font-bold flex items-center gap-1 disabled:opacity-40">{loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCheck className="w-3.5 h-3.5" />}تحديد الكل كمقروء</button></div>
      <div className="max-h-80 overflow-y-auto">{items.length === 0 ? <div className="p-8 text-center text-sm text-slate-400">لا توجد إشعارات بعد</div> : items.map((item) => <button key={item.id} onClick={() => !item.is_read && void markRead(item.id)} className={`w-full text-right p-3.5 border-b border-slate-50 hover:bg-slate-50 flex gap-3 ${!item.is_read ? 'bg-emerald-50/50' : ''}`}><span className={`mt-1 w-2 h-2 rounded-full shrink-0 ${item.is_read ? 'bg-slate-200' : 'bg-emerald-500'}`} /><span className="min-w-0"><strong className="block text-xs font-black">{item.title}</strong><span className="block text-xs text-slate-600 mt-1 leading-5">{item.message}</span><small className="block text-[10px] text-slate-400 mt-1">{relativeTime(item.created_at)}</small></span></button>)}</div>
    </div>}
  </div>;
};
